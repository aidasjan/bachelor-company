...
<div class='container'>
    <div class='row'>
        <div class='col container_grey py-5'>
            ...
            @if ($import_results['errors']->count() > 0)
                <div class='text-left pt-4 pb-2'>
                    <h2 class='font-weight-bold'>Errors ({{$import_results['errors']->count()}})</h2>
                ...
                <table class="table table_main table_error">
                    ...
                    @foreach ($import_results['errors'] as $key=>$error_messages)
            ...
            @if ($import_results['info']->count() > 0)
                <div class='text-left pt-4 pb-2'>
                    <h2 class='text-left font-weight-bold'>Successful Imports ({{$import_results['info']->count()}})</h2>
                </div>
                <table class="table table_main">
                    <tr><th>Row Number</th><th>Result</th></tr>
                    @foreach ($import_results['info'] as $key=>$info_messages)
